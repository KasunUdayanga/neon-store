...
export default function Cart() {
  const { items, cartCount, cartTotal, isCartOpen, setIsCartOpen, removeFromCart, updateQuantity } = useCart();
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-full sm:w-[450px] bg-white dark:bg-zinc-900 shadow-2xl z-[101] transform transition-transform duration-300 flex flex-col ${isCartOpen ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950/50">
          <button onClick={() => setIsCartOpen(false)} className="text-gray-500 hover:text-neon-pink transition-colors">
            <ArrowRight size={24} />
          </button>
        ...
        <div className="flex-grow overflow-y-auto p-6 flex flex-col items-center justify-center">
          {cartCount === 0 ? (
            ...
            <div className="w-full space-y-6 flex-col flex justify-start h-full">
              {items.map((item) => (
                <div key={item.id} className="flex flex-col border-b border-gray-100 dark:border-zinc-800 pb-5">
                  
                  {/* Top Row: Title, Details, and Delete */}
                  <div className="flex justify-between items-start mb-4">
                    <div className="pr-4">
                      <h3 className="font-bold text-gray-900 dark:text-white text-lg">{item.name}</h3>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 leading-relaxed line-clamp-2">{item.details}</p>
                    </div>
                    <button 
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4 border border-gray-200 dark:border-zinc-700 rounded-full px-3 py-1.5 bg-gray-50 dark:bg-zinc-800/50">
                      <button 
                        ...
                        className={`text-gray-500 hover:text-neon-pink transition p-1 ${item.quantity <= 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        <div className="p-6 border-t border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950">
          <div className="flex justify-between items-center mb-6">
            <span className="font-bold text-lg text-gray-600 dark:text-gray-300">Total</span>
            <span className="font-black text-2xl text-gray-900 dark:text-white">${cartTotal.toFixed(2)}</span>
          </div>
          ...
          >
            {cartCount === 0 ? "Shop Now" : "Proceed to Checkout"} <ArrowRight size={18} />
          </button>
        </div>
      </div>
    </>
